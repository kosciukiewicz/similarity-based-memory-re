...
from memory_re.data.sampling import create_joint_inference_sample, create_train_sample
from memory_re.utils.io import read_json_file


class MemoryReDataModule(pl.LightningDataModule):
    def __init__(
        self,
        dataset_name: str,
        ...
        self._neg_rel_count = neg_relation_count
    ...
    def _process_train_sample(self, doc: TokenizedDocument) -> dict[str, torch.Tensor]:
        sample = create_train_sample(
            ...
            rel_type_count=len(self._relation_types),
            neg_coref_count=self._neg_coref_count,
            neg_rel_count=self._neg_rel_count,
        )
        sample['doc_ids'] = torch.tensor(doc.doc_id, dtype=torch.long)

        return sample

    def _process_eval_sample(self, doc: TokenizedDocument) -> dict[str, torch.Tensor]:
        sample = create_joint_inference_sample(doc, self._max_span_size)
        sample['doc_ids'] = torch.tensor(doc.doc_id, dtype=torch.long)
